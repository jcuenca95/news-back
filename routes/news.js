...
const { validate, ValidationError, Joi, validator } = require('express-validation')
...
router.get('/', async (req, res, next) => {
    try {
        var result = await newsModel.findMany()
        res.json(result)
    } catch (error) {
...
router.post('/', validate(newsValidationPost, {}, {}), async (req, res, next) => {
    try {
...
router.put('/:id', validate(newsValidationPut, {}, {}), async (req, res, next) => {
    try {
